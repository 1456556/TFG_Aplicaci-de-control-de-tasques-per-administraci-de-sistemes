...
public class ConsultesTasques extends Conexio {
    ...
    public void GrupAfectat(JFPrincipal2 vis, int i) {
    ...
    public void UsuariAssignat(JFPrincipal2 vis, int i) {
    ...
    public void UsuariAssignat3(JFPrincipal2 vis, int i) {

        try {

            Connection cn = getConexio();
            PreparedStatement pst = cn.prepareStatement("select * from Usuaris where nivell = 'Administrador'");
            ResultSet rs = pst.executeQuery();
    ...
    public void UsuariAssignat1(JFPrincipal2 vis, int i) {
            ...
            if (repeticio > 0) {
                ...
                if (result < 0) {
                    ...
                    try {
                        ...
                        Connection cn2 = getConexio();
                        PreparedStatement pst2 = cn2.prepareStatement("insert into Tasques (titol, prioritat, usuari, data, estat, descripcio, setNotificacio, repeticioInici, dataFinal, recurrent, tipus, diesSetmana, grup, msg) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                        ...
                        pst2.setString(12, diesSetmana);
                        pst2.setString(13, grup);
                        Blob blob = (Blob) cn2.createBlob();
                        String aux = "";
                        blob.setBytes(1, aux.getBytes());
                        pst2.setBlob(14, blob);
                        pst2.executeUpdate();
    ...
    public int NovaTasca(Tasques tas) {
        ...
        System.out.println("DAY" + UsuariArrayList.get(0) + "MONTH" + titol + "YEAR" + date + "sdsd" + time + "sdsd" + descripcio);
        if (validacio == 0) {
            ...
            try {
                ...
                Connection cn2 = getConexio();
                PreparedStatement pst2 = cn2.prepareStatement("insert into Tasques (titol, prioritat, usuari, data, estat, descripcio, notificacio,  setNotificacio, grup, msg ) values (?,?,?,?,?,?,?,?,?,?)");
